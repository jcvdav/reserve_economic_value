...
prices <-
  ...
  filter(group == "Invertebrate" | family %in% families) %>% 
  mutate(family = ifelse(family %in% c("Serranidae", "Polyprionidae"),
                         "Serranidae and Polyprionidae", family)) %>% 
  distinct()
...
ts_plot <-
  ...
  stat_summary(geom = "line",
               ...
               linewidth = 0.5) +
  labs(x = "Year",
       y = bquote("Mean annual CPI-adjusted ex-vessel price (MXN"[2019] ~ "/ Kg)")) +
...
values_plot <- prices %>%
  ...
  labs(x = "Family",
       y = bquote("Mean CPI-adjusted ex-vessel price (MXN"[2019] ~ "/ Kg)")) +
  theme_bw() +
...
ggsave(
  plot = prices_panel,
  filename = here("results", "img", "fig_3_prices_panel.png"),
  width = 6,
  height = 8
)
